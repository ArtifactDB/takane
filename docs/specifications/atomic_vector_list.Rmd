...
The `atomic_vector_list` format provides an on-disk representation of a list of typed arrays.
...
- `version`, a string specifying the version of the `atomic_vector_list` file format.
```{r, results="asis", echo=FALSE}
...
Each entry corresponds to an element of the atomic vector list - namely, an atomic vector -
and defines the number of consecutive elements in the `concatenated` object (see below) that can be used to construct the atomic vector.
