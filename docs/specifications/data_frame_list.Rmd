...
This is based on the `CompressedList` subclasses for data frames, as implemented in the [**IRanges** Bioconductor package](https://bioconductor.org/packages/IRanges).

## Metadata

The top-level object in the `OBJECT` file should contain an `data_frame_list` property.
...
Optionally, the `data_frame_list` group may contain `names`, a 1-dimensional string dataset of the same dimensions as `values`.
