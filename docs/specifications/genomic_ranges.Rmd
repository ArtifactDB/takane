...
The top-level object in the `OBJECT` file should contain an `genomic_ranges` property.
...
The length of each dataset represents the number of genomic intervals.
...
If a reference sequence has an enforcible length, all intervals assigned to that sequence (as per their values in `genomic_ranges/sequence`) should have `start >= 1` and `start + width - 1` no greater than the sequence length.
...
The directory may contain an `range_annotations` subdirectory, which contains a child object that satisfies the `DATA_FRAME` interface.
