...
The top-level object in the `OBJECT` file should contain an `genomic_ranges_list` property.
...
The directory should also contain a `concatenated` subdirectory, which contains a child object of the `genomic_ranges` type.
...
Optionally, the `genomic_ranges_list` group may contain `names`, a 1-dimensional string dataset of the same dimensions as `values`.
