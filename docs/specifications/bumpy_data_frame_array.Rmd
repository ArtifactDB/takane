...
The top-level object in the `OBJECT` file should contain an `bumpy_data_frame_array` property.
...
Each entry corresponds to an entry of the bumpy atomic array - that is, an data frame -
...
The directory should also contain a `concatenated` subdirectory, which contains a child object of the `data_frame_vector` type.
...
Each `bumpy_data_frame_array/indices/X` dataset should be 1-dimensional with length equal to `bumpy_data_frame_array/lengths`, and should have a type that can be exactly represented by a 64-bit unsigned integer.
...
The height of the bumpy atomic array is defined as the extent of the first dimension, as specified in `bumpy_data_frame_array/dimensions`.

## Dimensions

The dimensions of the bumpy atomic array are taken from `bumpy_data_frame_array/dimensions`.
