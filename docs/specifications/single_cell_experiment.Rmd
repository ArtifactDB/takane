...
The top-level object in the `OBJECT` file should contain a `single_cell_experiment` property, 
...
In addition, the top-level object should contain all properties expected in a [`ranged_summarized_experiment`](ranged_summarized_experiment) object's metadata.
This includes the fields that are expected of a [`summarized_experiment`](summarized_experiment) object. 

## Files

The directory should contain all files expected of a [`ranged_summarized_experiment`](ranged_summarized_experiment) object.
This includes the transitive expectations of files that should be present in a [`summarized_experiment`](summarized_experiment) objects.
...
named after the positional index in the JSON array, e.g., `"0"` for the first result, `"1"` for the second result, and so on.
...
The `alternative_experimentst` subdirectory should contain one subdirectory for each alternative experiment,
named after the positional index in the JSON array, e.g., `"0"` for the first experiment, `"1"` for the second experiment, and so on.
