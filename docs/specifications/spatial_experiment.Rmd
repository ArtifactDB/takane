...
The top-level object in the `OBJECT` file should contain a `spatial_experiment` property, 
...
In addition, the top-level object should contain all properties expected in a [`single_cell_experiment`](single_cell_experiment) object's metadata.
This includes the transitive expectations of metadata fields from [`summarized_experiment`](summarized_experiment) and [`ranged_summarized_experiment`](ranged_summarized_experiment) object. 

## Files

The directory should contain all files expected of a [`single_cell_experiment`](single_cell_experiment) object.
This includes the transitive expectations of files that should be present in the [`summarized_experiment`](summarized_experiment) and [`summarized_experiment`](ranged_summarized_experiment) objects.
...
- `image_scale_factor`, a 1-dimensional dataset containing the scaling factor for each image.
  This should have the same length as `image_samples`.
  The datatype of this dataset should be exactly represented by a 64-bit float.
