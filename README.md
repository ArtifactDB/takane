...
- The version of this format is specified by the `hdf5_data_frame.version` property.
- The name of the group is specified by the `hdf5_data_frame.group` property. 
...
The types of atomic columns is determined from the `data_frame.columns.type` property in the schema:
...
- An integer column can be represented by any integer HDF5 dataset.
  Any integer data type can be used at the discretion of the data generator, though the type's range of values must be representable by a 32-bit signed integer.
- A factor column can be represented by any integer HDF5 dataset.
  Any integer data type can be used at the discretion of the data generator, though the type's range of values must be representable by a 32-bit signed integer.
...
**Version 1:**
- Missing integers and booleans were represented by -2147483648. 
- Missing floats were always represented by NaNs with R's missingness payload.
- Factors were stored as 1-dimensional string datasets rather than integer datasets of indices.
  Each entry in the string dataset should either be the missing placeholder (if the `missing-value-placeholder` was present) or found in the set of levels.
</details>
